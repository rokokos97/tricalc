...
const SwimBlock: React.FC = () => {
  ...
  const handleSubmit = (): void => {
    if (isPaceDisabled) {
      const result: string[] = paceCalculate(distance / 100, time)
      setPace(result)
      setResult({ pace: result })
    } else {
      const result: string[] = timeCalculate(distance / 100, pace)
